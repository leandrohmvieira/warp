...
impl<'a> TabComponent<'a> {
    pub fn new(
        ...
        let appearance = Appearance::as_ref(ctx);
        // Prefer a manual rename, then the CLI-agent session's own live title
        // (so Claude tabs show the session name, not the frozen launch command),
        // then finally the terminal-derived title.
        let title = {
            let pane_group = tab.pane_group.as_ref(ctx);
            pane_group
                .custom_title(ctx)
                .or_else(|| pane_group.agent_session_title(ctx))
