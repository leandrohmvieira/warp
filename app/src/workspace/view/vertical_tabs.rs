...
fn compute_tab_group_color_mode(
    ...
    // CLI-agent session running in them. Directory colors are suppressed, but a
    // color the user picked on this tab by hand still applies to tabs where no
    // pane has a tracked CLI-agent session — same precedence as the horizontal
    // tab bar (see `status_tab_color` in tab.rs).
    if matches!(
        *TabSettings::as_ref(app).tab_coloring_mode.value(),
        TabColoringMode::TerminalStatus
    ) {
        let status_mode = compute_status_tab_color_mode(pane_group, visible_pane_ids, theme, app);
        if !matches!(status_mode, TabGroupColorMode::None) {
            return status_mode;
        }
        return match tab.selected_color.resolve(None) {
            ...
            let color = pane_group
                ...
                .and_then(|tv| {
                    ...
                    let id = match &session.status {
                        ...
                        // Same grouping as the horizontal tab bar: Blocked and
                        // Failed both demand attention; Cancelled is at rest.
